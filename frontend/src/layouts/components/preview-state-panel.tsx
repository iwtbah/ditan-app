...
import { usePreviewStateContext, type PreviewAppState, type PreviewSubState } from "@/contexts/preview-state-context";

export function PreviewStatePanel() {
  const { appState, setAppState, subState, setSubState } = usePreviewStateContext();
      ...
      <div>
        ...
        <div className="mb-8">
          ...
          <div className="flex flex-col gap-3 px-2">
            {(["Normal", "Loading", "Empty", "Error"] as PreviewAppState[]).map((state) => (
        ...
        <div>
          ...
          <div className="flex flex-col gap-3 px-2">
            {(["Normal", "Loading", "Empty"] as PreviewSubState[]).map((state) => (
              <label key={state} className="group flex cursor-pointer items-center gap-3 text-sm">
                <input
