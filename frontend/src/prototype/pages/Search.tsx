...
import { FeedCard, ShopCard, ListContainer } from "../components/ditan";
...
const mockSearchNotes = [
  ...
  { id: 2, title: "绝美日落观景台，情侣约会必去", author: "浪漫主义者", likes: 1120, height: "h-40", liked: true },
];

const mockSearchShops = [
...
export const Search = () => {
  ...
  const [activeTab, setActiveTab] = useState("日记"); // 日记 or 店铺
    ...
    return (
      <div className="flex gap-sm mt-md">
        <div className="flex-1 flex flex-col gap-sm">
          {[1, 2].map(i => (
            <div key={i} className="bg-card rounded-lg overflow-hidden flex flex-col shadow-card border border-border p-md">
    ...
    <div className="flex flex-col h-full bg-muted">
      ...
      <div className="pt-safe bg-background z-10 sticky top-0 shadow-sm flex flex-col">
        <div className="flex items-center px-sm py-md gap-sm border-b border-border">
          <button onClick={() => navigate(-1)} className="p-sm text-text-secondary active:bg-muted rounded-full">
            <ChevronLeft size={24} />
          </button>
          
          <form onSubmit={handleSearch} className="flex-1 bg-muted h-9 rounded-full flex items-center px-md gap-sm">
          ...
          <div className="flex items-center justify-center gap-2xl border-b border-border pt-sm bg-background">
            <div 
              ...
              className={`pb-sm text-body relative cursor-pointer transition-colors ${
                activeTab === "日记" ? "text-text-primary font-bold" : "text-text-secondary hover:text-text-primary"
              }`}
            >
              日记
              {activeTab === "日记" && (
            ...
            <div 
              ...
              className={`pb-sm text-body relative cursor-pointer transition-colors ${
                activeTab === "店铺" ? "text-text-primary font-bold" : "text-text-secondary hover:text-text-primary"
              }`}
            >
              店铺
              {activeTab === "店铺" && (
            ...
            <div className="mt-md">
              {activeTab === "日记" ? (
                <div className="flex gap-sm">
                  {/* Left Column */}
                  <div className="flex-1 flex flex-col gap-sm">
                    {mockSearchNotes.filter((_, i) => i % 2 === 0).map(note => (
                      <FeedCard 
                        key={note.id} 
                        id={note.id}
                        title={note.title}
