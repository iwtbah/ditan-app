...
export const FeedCard = ({
  ...
  <Link
    to={id ? ROUTE_PATHS.noteDetail(String(id)) : "#"}
    onClick={onClick}
  ...
  >
    {image ? (
      <div className={`w-full relative overflow-hidden bg-muted ${imageClassName}`}>
        <img
          src={image}
          alt={title}
      ...
      <ImagePlaceholder className={`w-full ${imageClassName}`} />
    )}
      ...
      <div className="flex items-center justify-between mt-1">
        <div className="flex items-center gap-1.5">
          {authorAvatar ? (
            ...
            <AvatarPlaceholder size={16} className="opacity-80" />
          )}
          <span className="text-[11px] font-medium text-text-secondary truncate max-w-[60px]">
            {author}
          </span>
        </div>
        <LikeButton liked={liked} count={likes} className="scale-90 opacity-80" />
      </div>
    </div>
  </Link>
);

type ShopCardProps = ShopCardData & {
  className?: string;
  onClick?: React.MouseEventHandler<HTMLAnchorElement>;
};
...
export const ShopCard = ({
  ...
  <Link
    to={id ? ROUTE_PATHS.shopDetail(String(id)) : "#"}
    onClick={onClick}
      ...
      <div>
        ...
        <div className="flex items-center text-[12px] font-medium text-text-secondary mt-[3px] flex-wrap gap-x-2 gap-y-1">
          <span className="text-warning flex items-center font-bold">
            <Star size={11} className="mr-[3px]" fill="currentColor" strokeWidth={0} />
            {rating}
          </span>
          {price && <span className="text-text-tertiary">{price}</span>}
        </div>
      </div>
...
type CommentItemProps = NoteComment & {
  avatarSize?: number;
...
export const CommentItem = ({
  ...
  <div className="flex gap-md">
    ...
    <div className="flex-1 border-b border-border pb-md">
      ...
      <p className="text-body text-text-primary mt-sm">{content}</p>
      {children}
